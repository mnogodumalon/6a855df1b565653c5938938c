/**
 * HundekarteiDialog — pre-generated create/edit dialog for Hundekartei.
 *
 ...
 * HundekarteiDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Hundekartei['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<HundekarteiDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Hundekartei, Besitzer, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Hundekartei';
...
/** Widened prefill type for HundekarteiDialog.defaultValues — see file header. */
export type HundekarteiDialogDefaults = Omit<Hundekartei['fields'], 'impfstatus' | 'geschlecht'> & {
    impfstatus?: LookupValue | string;
    geschlecht?: LookupValue | string;
  };

interface HundekarteiDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Hundekartei['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: HundekarteiDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  impfstatus: LOOKUP_OPTIONS['hundekartei']?.['impfstatus'] ?? [],
  geschlecht: LOOKUP_OPTIONS['hundekartei']?.['geschlecht'] ?? [],
};
...
export function HundekarteiDialog({ open, onClose, onSubmit, defaultValues, recordId, besitzerList, enablePhotoScan = true, enablePhotoLocation = true }: HundekarteiDialogProps) {
  const [fields, setFields] = useState<Partial<Hundekartei['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['besitzer', 'name'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Hundekartei['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'hundekartei');
      await onSubmit(clean as Hundekartei['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Hundekartei['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('hundekartei') })
    : t('new_entity', { entity: appLabel('hundekartei') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">{fieldLabel('hundekartei', 'foto')}</Label>
        {fields.foto ? (
    ...
    'besitzer': (
      <div key="besitzer" className="space-y-1.5">
        <Label htmlFor="besitzer">{fieldLabel('hundekartei', 'besitzer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="besitzer"
    ...
    'impfstatus': (
      <div key="impfstatus" className="space-y-1.5">
        <Label htmlFor="impfstatus">{fieldLabel('hundekartei', 'impfstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('hundekartei', 'impfstatus', 'vollstaendig') ?? 'Vollständig geimpft'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'impfstatus', 'teilweise') ?? 'Teilweise geimpft'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'impfstatus', 'nicht_geimpft') ?? 'Nicht geimpft'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'impfstatus', 'unbekannt') ?? 'Unbekannt'}
          </button>
        </div>
      </div>
    ),
    'gesundheitshinweise': (
      <div key="gesundheitshinweise" className="space-y-1.5">
        <Label htmlFor="gesundheitshinweise">{fieldLabel('hundekartei', 'gesundheitshinweise')}</Label>
        <Textarea
          id="gesundheitshinweise"
          placeholder="Erkrankungen, Unverträglichkeiten, Medikamente..."
    ...
    'fuetterungshinweise': (
      <div key="fuetterungshinweise" className="space-y-1.5">
        <Label htmlFor="fuetterungshinweise">{fieldLabel('hundekartei', 'fuetterungshinweise')}</Label>
        <Textarea
          id="fuetterungshinweise"
          placeholder="Fütterungszeiten, Futter-Art, Portionsgröße..."
    ...
    'name': (
      <div key="name" className="space-y-1.5">
        <Label htmlFor="name">{fieldLabel('hundekartei', 'name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="name"
    ...
    'rasse': (
      <div key="rasse" className="space-y-1.5">
        <Label htmlFor="rasse">{fieldLabel('hundekartei', 'rasse')}</Label>
        <Input
          id="rasse"
    ...
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        <Label htmlFor="geburtsdatum">{fieldLabel('hundekartei', 'geburtsdatum')}</Label>
        <DatePicker
          id="geburtsdatum"
    ...
    'geschlecht': (
      <div key="geschlecht" className="space-y-1.5">
        <Label htmlFor="geschlecht">{fieldLabel('hundekartei', 'geschlecht')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'geschlecht', 'maennlich') ?? 'Männlich'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'geschlecht', 'weiblich') ?? 'Weiblich'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('hundekartei', 'geschlecht', 'unbekannt') ?? 'Unbekannt'}
          </button>
        </div>
      </div>
    ),
    'gewicht_kg': (
      <div key="gewicht_kg" className="space-y-1.5">
        <Label htmlFor="gewicht_kg">{fieldLabel('hundekartei', 'gewicht_kg')}</Label>
        <Input
          id="gewicht_kg"
          type="number"
          step="any"
  ...
  const FIELD_LABELS: Record<string, string> = {"foto": "Foto des Hundes", "besitzer": "Besitzer", "impfstatus": "Impfstatus", "gesundheitshinweise": "Gesundheitshinweise & Besonderheiten", "fuetterungshinweise": "Fütterungshinweise", "name": "Name des Hundes", "rasse": "Rasse", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "gewicht_kg": "Gewicht (kg)"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.HUNDEKARTEI} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
