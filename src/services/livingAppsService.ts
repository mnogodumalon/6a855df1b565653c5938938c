...
import type { Besitzer, Hundekartei, BelegungBuchungen, Buchungsanfragen, PfotenPortraet, WebsiteInhalte, CreateBesitzer, CreateHundekartei, CreateBelegungBuchungen, CreateBuchungsanfragen, CreatePfotenPortraet, CreateWebsiteInhalte } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- HUNDEKARTEI ---
  static async getHundekartei(): Promise<Hundekartei[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HUNDEKARTEI}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Hundekartei[];
    return hydrateRecords(records, 'hundekartei');
  }
  static async getHundekarteiEntry(id: string): Promise<Hundekartei | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HUNDEKARTEI}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Hundekartei;
    return hydrateRecords([record], 'hundekartei')[0];
  }
  static async createHundekarteiEntry(fields: CreateHundekartei): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.HUNDEKARTEI}/records`, { fields: cleanFieldsForApi(fields as any, 'hundekartei') });
    return { ...data, record_id: data.id };
  }
  static async updateHundekarteiEntry(id: string, fields: Partial<CreateHundekartei>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.HUNDEKARTEI}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'hundekartei') });
    return { ...data, record_id: data.id };
  }
  static async deleteHundekarteiEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HUNDEKARTEI}/records/${id}`);
  }

  // --- BELEGUNG_BUCHUNGEN ---
  static async getBelegungBuchungen(): Promise<BelegungBuchungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BELEGUNG_BUCHUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as BelegungBuchungen[];
    return hydrateRecords(records, 'belegung_buchungen');
  }
  static async getBelegungBuchungenEntry(id: string): Promise<BelegungBuchungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BELEGUNG_BUCHUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as BelegungBuchungen;
    return hydrateRecords([record], 'belegung_buchungen')[0];
  }
  static async createBelegungBuchungenEntry(fields: CreateBelegungBuchungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.BELEGUNG_BUCHUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'belegung_buchungen') });
    return { ...data, record_id: data.id };
  }
  static async updateBelegungBuchungenEntry(id: string, fields: Partial<CreateBelegungBuchungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.BELEGUNG_BUCHUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'belegung_buchungen') });
    return { ...data, record_id: data.id };
  }
  static async deleteBelegungBuchungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BELEGUNG_BUCHUNGEN}/records/${id}`);
  }
  ...
  // --- WEBSITE_INHALTE ---
  static async getWebsiteInhalte(): Promise<WebsiteInhalte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WEBSITE_INHALTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as WebsiteInhalte[];
    return hydrateRecords(records, 'website_inhalte');
  }
  static async getWebsiteInhalteEntry(id: string): Promise<WebsiteInhalte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WEBSITE_INHALTE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as WebsiteInhalte;
    return hydrateRecords([record], 'website_inhalte')[0];
  }
  static async createWebsiteInhalteEntry(fields: CreateWebsiteInhalte): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.WEBSITE_INHALTE}/records`, { fields: cleanFieldsForApi(fields as any, 'website_inhalte') });
    return { ...data, record_id: data.id };
  }
  static async updateWebsiteInhalteEntry(id: string, fields: Partial<CreateWebsiteInhalte>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.WEBSITE_INHALTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'website_inhalte') });
    return { ...data, record_id: data.id };
  }
  static async deleteWebsiteInhalteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WEBSITE_INHALTE}/records/${id}`);
  }
