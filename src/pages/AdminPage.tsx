...
import { HundekarteiViewDialog } from '@/components/dialogs/HundekarteiViewDialog';
import { BelegungBuchungenDialog } from '@/components/dialogs/BelegungBuchungenDialog';
import { BelegungBuchungenViewDialog } from '@/components/dialogs/BelegungBuchungenViewDialog';
...
import { WebsiteInhalteViewDialog } from '@/components/dialogs/WebsiteInhalteViewDialog';
...
const BELEGUNGBUCHUNGEN_FIELDS = [
  { key: 'hund', label: 'Hund', type: 'applookup/select', targetEntity: 'hundekartei', targetAppId: 'HUNDEKARTEI', displayField: 'name' },
  { key: 'besitzer', label: 'Besitzer', type: 'applookup/select', targetEntity: 'besitzer', targetAppId: 'BESITZER', displayField: 'vorname' },
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'aktiv', label: 'Aktiv' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }, { key: 'storniert', label: 'Storniert' }] },
  { key: 'preis_euro', label: 'Preis (€)', type: 'number' },
  { key: 'notizen', label: 'Notizen zur Buchung', type: 'string/textarea' },
];
const BUCHUNGSANFRAGEN_FIELDS = [
  ...
  { key: 'nachricht', label: 'Nachricht / Besondere Hinweise', type: 'string/textarea' },
  { key: 'anfrage_status', label: 'Status der Anfrage', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'abgelehnt', label: 'Abgelehnt' }] },
];
const PFOTENPORTRAET_FIELDS = [
  ...
  { key: 'charakterbeschreibung', label: 'Charakterbeschreibung', type: 'string/textarea' },
  { key: 'lieblingsaktivitaet', label: 'Lieblingsaktivität', type: 'string/text' },
  { key: 'besondere_momente', label: 'Besondere Momente beim Aufenthalt', type: 'string/textarea' },
  { key: 'portraet_foto', label: 'Foto für das Porträt', type: 'file' },
  { key: 'erstellungsdatum', label: 'Erstellungsdatum', type: 'date/date' },
];
const WEBSITEINHALTE_FIELDS = [
  { key: 'unternehmensname', label: 'Name der Hundepension', type: 'string/text' },
  { key: 'slogan', label: 'Slogan', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung des Angebots', type: 'string/textarea' },
  { key: 'anzahl_plaetze', label: 'Anzahl der Plätze', type: 'number' },
  { key: 'leistungen', label: 'Leistungen & Besonderheiten', type: 'string/textarea' },
  ...
  { key: 'galerie', label: 'Fotos für die Galerie', type: 'file' },
  { key: 'instagram', label: 'Instagram-Profil', type: 'string/url' },
  { key: 'facebook', label: 'Facebook-Seite', type: 'string/url' },
];

const ENTITY_TABS = [
  { key: 'besitzer', pascal: 'Besitzer' },
  { key: 'hundekartei', pascal: 'Hundekartei' },
  { key: 'belegung_buchungen', pascal: 'BelegungBuchungen' },
  { key: 'buchungsanfragen', pascal: 'Buchungsanfragen' },
  { key: 'pfoten_portraet', pascal: 'PfotenPortraet' },
  { key: 'website_inhalte', pascal: 'WebsiteInhalte' },
] as const;
...
export default function AdminPage() {
  ...
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'besitzer': new Set(),
    'hundekartei': new Set(),
    'belegung_buchungen': new Set(),
    'buchungsanfragen': new Set(),
    'pfoten_portraet': new Set(),
    'website_inhalte': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'hundekartei': return (data as any).hundekartei as Hundekartei[] ?? [];
      case 'belegung_buchungen': return (data as any).belegungBuchungen as BelegungBuchungen[] ?? [];
      ...
      case 'website_inhalte': return (data as any).websiteInhalte as WebsiteInhalte[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'hundekartei':
        ...
        break;
      case 'belegung_buchungen':
        lists.hundekarteiList = (data as any).hundekartei ?? [];
        lists.besitzerList = (data as any).besitzer ?? [];
        break;
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'hundekartei' && fieldKey === 'besitzer') {
      const match = (lists.besitzerList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'belegung_buchungen' && fieldKey === 'hund') {
      const match = (lists.hundekarteiList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.name ?? '—';
    }
    if (entity === 'belegung_buchungen' && fieldKey === 'besitzer') {
      const match = (lists.besitzerList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        ...
        case 'hundekartei': return HUNDEKARTEI_FIELDS as any[];
        case 'belegung_buchungen': return BELEGUNGBUCHUNGEN_FIELDS as any[];
        case 'buchungsanfragen': return BUCHUNGSANFRAGEN_FIELDS as any[];
        case 'pfoten_portraet': return PFOTENPORTRAET_FIELDS as any[];
        case 'website_inhalte': return WEBSITEINHALTE_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'hundekartei': return {
        create: (fields: any) => LivingAppsService.createHundekarteiEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateHundekarteiEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteHundekarteiEntry(id),
      };
      case 'belegung_buchungen': return {
        create: (fields: any) => LivingAppsService.createBelegungBuchungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBelegungBuchungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBelegungBuchungenEntry(id),
      };
      ...
      case 'website_inhalte': return {
        create: (fields: any) => LivingAppsService.createWebsiteInhalteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWebsiteInhalteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWebsiteInhalteEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'hundekartei' || dialogState?.entity === 'hundekartei') && (
        <HundekarteiDialog
          open={createEntity === 'hundekartei' || dialogState?.entity === 'hundekartei'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'hundekartei' ? handleUpdate : (fields: any) => handleCreate('hundekartei', fields)}
          defaultValues={dialogState?.entity === 'hundekartei' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'belegung_buchungen' || dialogState?.entity === 'belegung_buchungen') && (
        <BelegungBuchungenDialog
          open={createEntity === 'belegung_buchungen' || dialogState?.entity === 'belegung_buchungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'belegung_buchungen' ? handleUpdate : (fields: any) => handleCreate('belegung_buchungen', fields)}
          defaultValues={dialogState?.entity === 'belegung_buchungen' ? dialogState.record?.fields : undefined}
          hundekarteiList={(data as any).hundekartei ?? []}
          besitzerList={(data as any).besitzer ?? []}
      ...
      {(createEntity === 'website_inhalte' || dialogState?.entity === 'website_inhalte') && (
        <WebsiteInhalteDialog
          open={createEntity === 'website_inhalte' || dialogState?.entity === 'website_inhalte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'website_inhalte' ? handleUpdate : (fields: any) => handleCreate('website_inhalte', fields)}
          defaultValues={dialogState?.entity === 'website_inhalte' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'hundekartei' && (
        <HundekarteiViewDialog
          open={viewState?.entity === 'hundekartei'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'hundekartei', record: r }); }}
          besitzerList={(data as any).besitzer ?? []}
        />
      )}
      {viewState?.entity === 'belegung_buchungen' && (
        <BelegungBuchungenViewDialog
          open={viewState?.entity === 'belegung_buchungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'belegung_buchungen', record: r }); }}
          hundekarteiList={(data as any).hundekartei ?? []}
          besitzerList={(data as any).besitzer ?? []}
        />
      )}
      ...
      {viewState?.entity === 'website_inhalte' && (
        <WebsiteInhalteViewDialog
          open={viewState?.entity === 'website_inhalte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'website_inhalte', record: r }); }}
        />
      )}
