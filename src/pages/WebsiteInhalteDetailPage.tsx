...
import { formEnhancements } from '@/config/form-enhancements/WebsiteInhalte';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function WebsiteInhalteDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<WebsiteInhalte | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getWebsiteInhalte();
  ...
  async function handleUpdate(fields: WebsiteInhalte['fields']) {
    if (!record) return;
    await LivingAppsService.updateWebsiteInhalteEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteWebsiteInhalteEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/website-inhalte');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/website-inhalte')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/website-inhalte')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.unternehmensname ?? appLabel('website_inhalte')} />
