...
export default function HundekarteiPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Hundekartei[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Hundekartei | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Hundekartei | null>(null);
  ...
  async function handleCreate(fields: Hundekartei['fields']) {
  ...
  async function handleUpdate(fields: Hundekartei['fields']) {
    ...
    await LivingAppsService.updateHundekarteiEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteHundekarteiEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('hundekartei') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('hundekartei') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('besitzer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'besitzer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('impfstatus')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'impfstatus')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('gesundheitshinweise')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'gesundheitshinweise')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('fuetterungshinweise')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'fuetterungshinweise')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('geburtsdatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'geburtsdatum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('geschlecht')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'geschlecht')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('gewicht_kg')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('hundekartei', 'gewicht_kg')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/hundekartei/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('hundekartei', 'impfstatus', record.fields.impfstatus?.key) ?? record.fields.impfstatus?.label ?? '—'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('hundekartei', 'geschlecht', record.fields.geschlecht?.key) ?? record.fields.geschlecht?.label ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('hundekartei') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
